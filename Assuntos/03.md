...
4. Altere novamente o programa, o vetor agora deve ser de structs contendo pares de inteiros:
```c
typedef struct {
  int a;
  int b;
...
```
A função `main` deve alocar o vetor de pares, do tamanho pedido pelo usuário, e então chamar uma função para preencher todo o vetor.
Essa função, por sua vez deve chamar uma outra função, para preencher cada par, que deve ser declarada assim: `void preenche_par(par *p)`.
A função `preenche_par` deve preencher o par chamando a função `aleat` (que não deve ser alterada) duas vezes, e garantindo que o par contém dois números entre 1 e 100, e o segundo número obrigatoriamente deve ser maior que o primeiro.
