...
No final, chegamos a uma versão do programa que implementa o que é pedido no terceiro exercicio:
...
{
    // TODO: deixar 'de' etc em minusculas
    char *p = nome;
...
Para isso, você deve criar o tipo `vetor_de_linhas`, que é usado por todas as funções chamadas por `main`, e deve concentrar as informações que essas funções necessitam (vai ser uma struct).
