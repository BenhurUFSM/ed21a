...
Qual o menos caminho entre duas cidades? 
...
Tal arquivo foi alterado, quais devem ser recompilados em função dessa alteração?
...
Se, ao contrário, as arestas têm direção definida, o grafo é chamado de **orientado**, ou **dígrafo** (do inglês digraph -- directed graph). Cada aresta é representada por um par ordenado de vértices, escrito com a identificação dos dois nós entre parênteses -- uma aresta de `a` para `b` é escrita assim: `(a,b)`.

Um grafo pode ser **ponderado** ou valorado, quando se tem um valor numérico associado a cada vértice ou a cada aresta. Esse valor pode representar, por exemplo, o custo para se percorrer a estrada representada por essa aresta.

Dois vértices são **adjacentes** ou vizinhos existe uma aresta ligando-os.
Diz-se que essa aresta é **incidente** a esses vértices.
Se é um dígrafo, o nó de partida da aresta é dito antecessor e o de chegada é o sucessor.
