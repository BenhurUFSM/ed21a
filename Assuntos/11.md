...
Falta fazer o módulo pelo tamanho da tabela. É uma função clássica de *hash*, polinomial (*hash(x) = x<sub>0</sub> + x<sub>1</sub>k + x<sub>2</sub> k<sup>2</sup> + x<sub>3</sub>k<sup>3</sup> ...*)
```c
  int hash(char *s)
  {
    int h=0;
